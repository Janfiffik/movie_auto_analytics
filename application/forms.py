...
class NewMovieForm(FlaskForm):
    ...
    Oscar_Wins = IntegerField("Number of oscars", validators=[Optional()], default=0)
    Oscar_Nomination = IntegerField("Number oscar nominations", validators=[Optional()], default=0)
    Other_Wins = IntegerField("Other Wins", validators=[Optional()], default=0)
    submit = SubmitField("Add movie to DataBase")


class UpdateMovieForm(FlaskForm):
    Imdb_Rating = FloatField("IMDb Rating", validators=[Optional()], default=0)
    Imdb_Votes = IntegerField("IMDb number of votes", validators=[Optional()], default=0)
    Gross_US = IntegerField("Gross income in USA", validators=[Optional()], default=0)
    World_Gross = IntegerField("Gross income in World", validators=[Optional()], default=0)
    Oscar_Wins = IntegerField("Number of oscars", validators=[Optional()], default=0)
    Oscar_Nomination = IntegerField("Number oscar nominations", validators=[Optional()], default=0)
    Other_Wins = IntegerField("Other Wins", validators=[Optional()], default=0)
